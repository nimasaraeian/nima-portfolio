...
/**
 * Backend base URL for Railway
 * Uses BACKEND_BASE_URL environment variable or falls back to Railway URL
 * NOTE: Replace <YOUR_RAILWAY_BACKEND_URL> with your actual Railway backend URL
 */
const BACKEND_BASE_URL =
  process.env.BACKEND_BASE_URL ?? "https://<YOUR_RAILWAY_BACKEND_URL>";
...
/**
 * Get backend URL - uses BACKEND_BASE_URL or falls back to local dev
 */
function getBackendUrl(): string {
  const url =
    BACKEND_BASE_URL !== "https://<YOUR_RAILWAY_BACKEND_URL>"
      ? BACKEND_BASE_URL
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      "🔍 Calling OpenAI Vision API to extract decision-critical text...",
    );
      ...
      return jsonResponse(
        {
          ...
          detail:
            "The image does not contain enough decision-critical information (pricing, CTAs, guarantees, etc.). Please upload an image with pricing, call-to-action buttons, guarantees, or other decision-affecting elements.",
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      "[Decision Engine Image] Forwarding extracted text to backend:",
      backendEndpoint,
    );
