...
function PageStructureCard({ page }: { page?: PageStructure | null }) {
  if (!page) {
    return null;
  }
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-900/40 p-4 space-y-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {entries.map(({ key, label }) => (
          <div key={key} className="rounded-lg border border-slate-700/80 bg-slate-950/50 p-3">
...
function getStructuredBlockers(
  result: CognitiveFrictionResult,
...
function ResultsPanel({ result }: { result: CognitiveFrictionResult }) {
  ...
  const quickWinAccent = {
    border: 'border-green-500/20',
    bg: 'bg-green-900/10',
    title: 'text-green-300',
    icon: '⚡',
  };
  const deepChangeAccent = {
    border: 'border-indigo-500/20',
    bg: 'bg-indigo-900/10',
    title: 'text-indigo-300',
    icon: '🔧',
  };
  const primaryDiagnosis = result.primary_diagnosis ?? result.explanationSummary;
  const analysisSummary = result.psychology_narrative?.analysis_summary;
  const visualTrustScore =
    typeof result.visual_trust_score === 'number' && !isNaN(result.visual_trust_score)
      ? result.visual_trust_score
      : null;
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <RecommendationSection
            title="Quick Wins"
            description="Immediate changes to reduce friction fast."
            structuredItems={quickWinItems}
            fallbackItems={result.recommendedQuickWins}
            accent={quickWinAccent}
          />

          <RecommendationSection
            title="Deep Changes"
            description="Structural and strategic improvements for long-term impact."
            structuredItems={deepChangeItems}
            fallbackItems={result.recommendedDeepChanges}
            accent={deepChangeAccent}
          />
        </div>
      </div>
        ...
        <div className="space-y-4">
          {primaryDiagnosis && (
            <div className="rounded-xl border border-purple-500/20 bg-purple-900/10 p-4">
              <h4 className="text-sm font-semibold mb-2 text-purple-200">Primary Diagnosis</h4>
              <p className="text-sm text-gray-300 leading-relaxed">{primaryDiagnosis}</p>
            </div>
          )}

          {analysisSummary && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-900/10 p-4">
              <h4 className="text-sm font-semibold mb-2 text-amber-200">Analysis Summary</h4>
