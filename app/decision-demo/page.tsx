...
export default function DecisionDemoPage() {
  ...
  const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUrl(newUrl);
    if (newUrl) {
      setContent(""); // Clear text when URL is entered
  ...
  const handleAnalyze = async () => {
    ...
    // If URL is provided, use URL endpoint
    if (url.trim()) {
      setIsLoading(true);
      try {
    ...
    // Basic validation - content should be substantial
      ...
      <div className="w-full max-w-3xl space-y-8">
        ...
        <header className="space-y-3">
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">
            Why users hesitate — and what to fix first.
          </h1>
          <p className="text-slate-300 max-w-xl">
            Paste the decision part of your page (price, CTA, form, size,
            guarantees…), upload a screenshot, or enter a URL, and get the ONE blocker and the FIRST fix.
          </p>
          <details className="text-sm text-slate-400">
            <summary className="cursor-pointer hover:text-slate-300">
              What content should I paste?
            </summary>
            <div className="mt-2 pl-4 space-y-2 text-xs">
              <div>
                <p className="font-medium mb-1">Content types:</p>
                <ul className="space-y-0.5">
                  <p>✓ Pricing tables or subscription plans</p>
                  <p>✓ "Buy Now" or "Sign Up" buttons with surrounding text</p>
                  <p>✓ Checkout forms or registration fields</p>
                  <p>✓ Guarantee, refund, or return policy text</p>
                  <p>✓ Product size/option selectors with descriptions</p>
                  <p>✓ Risk-reduction messaging (money-back, free trial, etc.)</p>
                </ul>
              </div>
              <div className="pt-2 border-t border-slate-700">
                <p className="font-medium mb-1">About URL input:</p>
                <p className="text-slate-500">
                  URL analysis works best with publicly accessible pages. Some websites block automated requests (CORS, bot protection). If URL fails, use text paste or image upload instead.
                </p>
              </div>
            </div>
          </details>
        ...
        <section className="space-y-3">
          <label className="block text-sm font-medium text-slate-200">
            Decision section
            {imageFile && <span className="text-slate-400 text-xs ml-2">(Image mode)</span>}
            {url && !imageFile && <span className="text-slate-400 text-xs ml-2">(URL mode)</span>}
          </label>
          ...
          <div className="space-y-2">
            <input
              ...
              className={`w-full rounded-xl border border-slate-700 bg-slate-900/60 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
            ...
            {url && !imageFile && !content && (
              <div className="text-xs text-slate-400 space-y-1">
                <p>Enter a URL to analyze the page content automatically</p>
                <p className="text-slate-500">
                  ⚠️ Note: Some websites block automated requests. If URL fails, copy the page content and paste it as text, or upload a screenshot.
                </p>
              </div>
            )}
          </div>
          ...
          {/* Image Upload */}
          <div className="space-y-2">
            <label className={`flex items-center justify-center w-full h-32 border-2 border-dashed rounded-xl bg-slate-900/40 transition ${
              url || content 
          ...
          <div className="relative">
            <textarea
              className={`w-full h-40 rounded-xl border border-slate-700 bg-slate-900/60 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-vertical ${
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <button
              ...
              disabled={isLoading}
              className="inline-flex items-center justify-center rounded-full px-6 py-2.5 text-sm font-medium bg-indigo-500 hover:bg-indigo-400 disabled:opacity-60 disabled:cursor-not-allowed transition"
            >
            ...
            {error && (
              <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 space-y-2">
                ...
                <p className="text-sm text-red-300">
                  {error.includes("Invalid decision_blocker") 
                    ? "The content you provided doesn't contain enough decision-critical information. Please paste content that includes pricing, CTAs, guarantees, forms, or other elements that affect user decisions."
                    : error}
                </p>
                {error.includes("Invalid decision_blocker") && (
                  <p className="text-xs text-red-400/80">
                    Tip: Include text about pricing, purchase buttons, guarantees, return policies, or checkout forms.
                  </p>
                )}
                {(error.includes("fetch failed") || error.includes("CORS") || error.includes("blocked") || error.includes("network")) && (
                  <div className="text-xs text-red-400/80 space-y-1 mt-2 pt-2 border-t border-red-500/20">
                    <p className="font-medium">Alternative solutions:</p>
                    <ul className="list-disc list-inside space-y-0.5 ml-2">
                      <li>Copy the page content manually and paste it in the text field below</li>
                      <li>Take a screenshot of the page and upload it as an image</li>
                      <li>Try a different URL if the website blocks automated requests</li>
                    </ul>
                  </div>
                )}
              </div>
        ...
        {result && (
          <section className="mt-6 space-y-4">
            <div className="border border-slate-800 rounded-2xl bg-slate-900/70 p-5 space-y-4">
              ...
              <div>
                ...
                <div className="mt-1 rounded-xl bg-slate-800/80 px-4 py-3 text-sm text-slate-50">
              ...
              <div className="flex items-center justify-between">
                ...
                <p className="text-[11px] text-slate-500 max-w-[220px] text-right">
                  Not a guarantee. This is a directional estimate based on
                  psychological friction, not analytics.
                </p>
              </div>
            </div>
          </section>
